...
export function createOrganizationLifecycleAtoms(): OrganizationLifecycleAtoms {
  ...
  const retireEffect: ActionEffect = async (tx, request, _objects, ctx) => {
    ...
    // Both keep their FIRST value. `retired_at` was already set on the nine organizations
    // mis-retired by SQL on 2026-09-10, and filtering on `retired_at is null` here lost the
    // successor on exactly those nine; a successor, once named, is not silently replaced by a
    // later act — there is no transition out of `retired`, so a later act is not expected, and
    // if one arrives the first answer stands.
    await tx.query(
      `update org.organization
          set retired_at = coalesce(retired_at, $2), succeeded_by = coalesce(succeeded_by, $3)
        where id = $1`,
